...
**Top tabs:** All · Manual · AI-generated · MCP · Failing — with counts. **AI-generated tab hidden in ZERO** (no AI-source cases will exist). Filter button + **split-button** "Generate" primary CTA + tier-agnostic "Test strategy" action.
...
- Effective testing-approach pill (Black-box/Gray-box/White-box); the search row
  has an approach filter, and suite headers show their configured default.

**Detail panel sections:**
1. Toolbar: ID badge, source, effective testing-approach badge, approach override,
   independent test-level select, status, priority, and actions (Delete, Run now)
...
**Test strategy dialog:** deterministic ZERO draft from repository,
observability, and internal-provider access signals; structured risk review;
validated JSON edit; approve/supersede. LOCAL/CLOUD also shows "Enrich with AI".
Approach and test level are always presented as separate fields.
...
3. Normalized line/branch coverage row when the lifecycle published coverage
4. Tabs: Logs · Steps · Artifacts · Browser · Network
5. Body: split between logs (left) and browser preview (right 280px)
